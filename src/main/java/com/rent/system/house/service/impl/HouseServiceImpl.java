...
public class HouseServiceImpl implements HouseService {
  ...
  public void getHouseType(String houseId, HttpServletResponse response) {
    ...
    if (optional.isPresent()) {
      ...
      if (StringUtils.hasLength(houseType)) {
  ...
  @Override
  public ResponseEntity<CommonHttpResponse<List<HouseBaseInfo>>> currentRentHouse(
      HttpSession session) {
    String userId = String.valueOf(session.getAttribute("userId"));
  ...
  @Override
  public ResponseEntity<CommonHttpResponse<Map<Integer, List<HouseBaseInfo>>>> tenantAuditInfo(
      HttpSession session) {
    String userId = String.valueOf(session.getAttribute("userId"));
    List<HouseEntity> houseEntities = houseDao.findByHouseTenant(userId);
    Map<Integer, List<HouseBaseInfo>> map = houseEntities.stream()
        .filter(entity -> entity.getHouseRentStatus() == 3 || entity.getHouseRentStatus() == 4
            || entity.getHouseRentStatus() == 6)
  ...
  @Override
  public ResponseEntity<CommonHttpResponse<Map<Integer, List<HouseBaseInfo>>>> adminAuditInfo(
      HttpSession session) {
    List<Integer> status = new ArrayList<>();
    status.add(0);
    status.add(1);
  ...
  public ResponseEntity<CommonHttpResponse<List<HouseBaseInfo>>> getRentHouse(HttpSession session) {
    ...
    return CommonHttpResponse.ok(entities.stream()
        ...
        .peek(info -> userDao.findById(info.getHouseTenant())
            .ifPresent(user -> info.setLodger(new LodgerInfo(user))))
        .collect(Collectors.toList()));
  }

  @Override
  public ResponseEntity<CommonHttpResponse<List<HouseBaseInfo>>> getOwnerRentHouse(
      HttpSession session) {
    String userId = String.valueOf(session.getAttribute("userId"));
    List<HouseEntity> entities = houseDao
