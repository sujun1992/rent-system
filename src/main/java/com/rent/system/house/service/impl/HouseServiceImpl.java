...
public class HouseServiceImpl implements HouseService {
  ...
  @Override
  public ResponseEntity<CommonHttpResponse<HouseBaseInfo>> currentRentHouse(HttpSession session) {
    String userId = String.valueOf(session.getAttribute("userId"));
    Optional<HouseEntity> optional = houseDao.findByHouseTenant(userId);
    if (optional.isPresent()) {
      HouseEntity entity = optional.get();
      return CommonHttpResponse.ok(new HouseBaseInfo(entity));
    }
  ...
  @Override
  public ResponseEntity<CommonHttpResponse<String>> leaseRenewal(HttpSession session) {
    String userId = String.valueOf(session.getAttribute("userId"));
    Optional<HouseEntity> optional = houseDao.findByHouseTenant(userId);
    if (optional.isPresent()) {
      HouseEntity entity = optional.get();
